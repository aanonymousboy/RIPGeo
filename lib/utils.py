...
class ParaPerturb:
    ...
    def random_attack(self, data):
        ...
        for (_, adv_param), (_, param) in zip(self.vice_model.named_parameters(), self.model.named_parameters()):
            adv_param.data = param.data + self.zeta * torch.normal(0, torch.ones_like(param.data) * param.data).cuda()
    ...
    def perturb(self, model, data):
        ...
        for t in range(self.step):
            ...
            attack_loss = self.attack_loss(data)
            if t != (self.step - 1):
