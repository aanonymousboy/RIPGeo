...
class RIPGeo(nn.Module):
    def __init__(self, dim_in, dim_z, dim_med, dim_out, collaborative_mlp=True):
        ...
        # RIPGeo
        self.att_attribute = SimpleAttention(temperature=dim_z ** 0.5,
                                             d_q_in=dim_in,
                                             d_k_in=dim_in,
                                             d_v_in=dim_in + 2,
                                             d_q_out=dim_z,
        ...
        if collaborative_mlp:
            self.pred = SimpleAttention(temperature=dim_z ** 0.5,
                                        d_q_in=dim_in * 3 + 4,
                                        d_k_in=dim_in,
                                        d_v_in=2,
                                        d_q_out=dim_z,
    ...
    def forward(self, lm_X, lm_Y, tg_X, tg_Y, lm_delay, tg_delay):
        ...
        '''visualize adjacent matrix 0 '''
        # from torchvision import transforms
        # unloader = transforms.ToPILImage()
        # image = adj_matrix_0.cpu().clone()  # clone the tensor
        # image = image.squeeze(0)  # remove the fake batch dimension
        # image = unloader(image)
        # image.save('step_1_adj_matrix.jpg')
        ...
        '''visualize adjacent matrix 1'''
        # from torchvision import transforms
        # unloader = transforms.ToPILImage()
        # image = adj_matrix_1.cpu().clone()  # clone the tensor
        # image = image.squeeze(0)  # remove the fake batch dimension
        # image = unloader(image)
        # image.save('step_2_adj_matrix.jpg')
